...
from sglang.srt.disaggregation.common.utils import (
    ...
    group_concurrent_contiguous_arrays,
    pack_int_lists,
    unpack_int_lists,
)
...
class NixlKVManager(CommonKVManager):
    ...
    def send_kvcache_slice(
        ...
        # Stack all K then all V layer pointers into a single vector so the
        # descriptor build is a single vectorized outer-product (no per-layer loop).
        L = layers_current_pp_stage
        src_ptrs_arr = np.empty(2 * L, dtype=np.uint64)
        dst_ptrs_arr = np.empty(2 * L, dtype=np.uint64)
        if L > 0:
            src_ptrs_arr[:L] = src_k_ptrs[:L]
            src_ptrs_arr[L:] = src_v_ptrs[:L]
            dst_ptrs_arr[:L] = dst_k_ptrs[:L]
            dst_ptrs_arr[L:] = dst_v_ptrs[:L]

        bytes_per_token_prefill = src_kv_item_len // page_size
        bytes_per_token_decode = dst_kv_item_len // page_size

        # Coalesce contiguous index pairs into blocks; each block becomes one
        # strided descriptor (counts repetitions, strides bytes apart) which a
        # stride-capable backend (UCX UMR) turns into a single SGL transfer.
        prefill_starts, dst_starts, block_lens = group_concurrent_contiguous_arrays(
            prefill_kv_indices, dst_kv_indices
        )
        num_blocks = prefill_starts.size
        per_block_counts = (block_lens * page_size).astype(np.uint64)

        src_off = prefill_starts.astype(np.uint64) * np.uint64(
            src_kv_item_len
        ) + np.uint64(src_head_slice_offset)
        dst_off = dst_starts.astype(np.uint64) * np.uint64(
            dst_kv_item_len
        ) + np.uint64(dst_head_slice_offset)

        total = (2 * L) * num_blocks
        src_reqs = np.empty((total, 3), dtype=np.uint64)
        dst_reqs = np.empty((total, 3), dtype=np.uint64)
        if total > 0:
            src_addrs_2d = src_ptrs_arr[:, None] + src_off[None, :]
            dst_addrs_2d = dst_ptrs_arr[:, None] + dst_off[None, :]
            src_reqs[:, 0] = src_addrs_2d.ravel()
            dst_reqs[:, 0] = dst_addrs_2d.ravel()
            src_reqs[:, 1] = heads_bytes_per_token_to_send
            src_reqs[:, 2] = self.kv_args.gpu_id
            dst_reqs[:, 1] = heads_bytes_per_token_to_send
            dst_reqs[:, 2] = dst_gpu_id

        # Only attach the counts/strides side-car when a block actually spans more
        # than one token; otherwise a plain per-descriptor list is sufficient.
        if total > 0 and (page_size > 1 or bool((block_lens > 1).any())):
            src_counts = np.tile(per_block_counts, 2 * L)
            dst_counts = src_counts
            src_strides = np.full(total, bytes_per_token_prefill, dtype=np.uint64)
            dst_strides = np.full(total, bytes_per_token_decode, dtype=np.uint64)
        else:
            src_counts = src_strides = None
            dst_counts = dst_strides = None

        src_descs = self.agent.get_xfer_descs(
            src_reqs, "VRAM", counts=src_counts, strides=src_strides
        )
        dst_descs = self.agent.get_xfer_descs(
            dst_reqs, "VRAM", counts=dst_counts, strides=dst_strides
        )
